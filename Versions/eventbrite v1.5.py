...
if __name__ == '__main__':
    ...
    while True:
        ...
        page_count = 1
        while True:
            req = sess.get(f'https://www.eventbrite.ca/d/canada--ontario/music--events/?page={page_count}',headers=get_headers())
            print(req.status_code,page_count)
            main_page = Selector(text=req.text)
            ...
            if len(ids_to_scrape) > 0:
                ...
                for row in req.json()['events']:
                    try:
                        ...
                        data['full_address'] = ', '.join(resp.xpath('//section[@aria-labelledby="location-heading"]/div[2]/p/text()').getall())
            ...
            if main_page.xpath('//button[@data-spec="page-next" and not(@disabled)]').get():
                page_count +=1
